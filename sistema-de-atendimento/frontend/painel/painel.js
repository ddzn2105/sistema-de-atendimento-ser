document.addEventListener("DOMContentLoaded", () => {
  ...
  async function atualizarPainel() {
    try {
      ...
      // Identifica novas senhas que ainda não foram exibidas
      const novasSenhas = ultimasChamadas.filter(
        (s) => !senhasExibidas.has(s.numero),
      );

      // Se houver novas senhas, mostra a mais recente
      if (novasSenhas.length > 0) {
        const senhaMaisRecente = novasSenhas[0];
        const prefixo =
        ...
        // Marca todas as senhas dessa atualização como exibidas
        ultimasChamadas.forEach((s) => senhasExibidas.add(s.numero));
      }

      // Atualiza o histórico (sem afetar o flash)
